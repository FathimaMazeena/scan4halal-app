...
function Hero() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // ✅ The matches are already in the response from /ocr
      // No need for second API call to /rag/retrieve
      setIngredientMatches(data.matches || []);
      
      console.log("Full response from OCR:", data);

    } catch (error) {
      console.error("Upload failed", error);
      setOcrText("Error occurred while processing image");
      setIngredientMatches([]); // Clear any previous matches on error
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {/* Show ingredient results if we have any matches */}
      {ingredientMatches.length > 0 && (
        <IngredientResults ingredientMatches={ingredientMatches} />
      )}
